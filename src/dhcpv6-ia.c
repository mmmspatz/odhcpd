...
static bool assign_pd(struct interface *iface, struct dhcpv6_lease *assign)
{
	...
	bool allow_exclude =
		...
		(assign->length < 64);	/* Delegated prefix must be shorter than /64 so the excluded /64 fits within it (RFC 6603 §4.2) */
		...
		for (size_t i = 0; i < addrlen; ++i) {
			...
			if (a->flags & OAF_DHCPV6_PD) {
				...
				if (a->assigned_subnet_id == 0) {
					/* netifd assigns the first /64 as the on-link prefix when it is delegating a prefix to an interface */
